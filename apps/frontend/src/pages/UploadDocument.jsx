...
import {
  ...
  Eye, Trash2, RefreshCw, FolderOpen, Sparkles
} from "lucide-react";
          ...
          {NAV.map((item) => (
            <div key={item.id}
              className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg cursor-pointer transition-all ${
                item.active ? "bg-blue-50 ring-1 ring-blue-100" : "hover:bg-slate-50"
              }`}>
              <item.icon size={15}
                className={item.active ? "text-blue-600" : "text-slate-400"}
                strokeWidth={item.active ? 2.5 : 1.8} />
              <div className="flex-1 min-w-0">
                <p className={`text-[11px] font-semibold truncate ${item.active ? "text-blue-700" : "text-slate-500"}`}>
                  {item.label}
                </p>
